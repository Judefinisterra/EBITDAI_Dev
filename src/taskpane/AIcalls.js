...
export async function handleInitialConversation(clientprompt) {
    ...
    if (ENCODER_API_TYPE === ENCODER_API_TYPES.RESPONSES) {
        ...
        // Check model dropdown for Claude model selection
        ...
        encoderModel = modelDropdown ? modelDropdown.value : "claude-opus-4-1-20250805";
        apiType = "Claude API";
    }
...
Office.onReady(async (info) => {
  if (info.host === Office.HostType.Excel) {
    ...
    async function showClientModeView() { // Renamed
      ...
      // Microsoft auth removed - no longer checking for msal_access_token
      const apiKeyLocal = localStorage.getItem('user_api_key');
      
      const isAuthenticated = (googleUserSession && googleCredentialSession) || 
                            !!(googleTokenLocal || apiKeyLocal);
      
      if (DEBUG) {
        console.log(`Authentication check - Session: ${!!(googleUserSession && googleCredentialSession)}, Local: ${!!(googleTokenLocal || apiKeyLocal)}`);
      }
      ...
      if (!hasBackendToken) {
        ...
        // Backend authentication removed - standalone mode
        console.log('Running in standalone mode - no backend authentication');
      }
    ...
    function buildGoogleAuthUrlView() {
      const GOOGLE_CLIENT_ID = ""; // Authentication removed
