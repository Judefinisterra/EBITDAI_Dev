...
export async function runCodes(codeCollection) {
    try {
        ...
        // Clear cells in column B that contain exactly "BR" on all assumption tabs
        console.log("[CLEANUP] Starting to clear BR cells in column B on all assumption tabs");
        for (const assumptionTab of assumptionTabs) {
            try {
                await Excel.run(async (clearContext) => {
                    const worksheet = clearContext.workbook.worksheets.getItem(assumptionTab.name);
                    
                    // Get used range to determine how many rows to check
                    const usedRange = worksheet.getUsedRange(true);
                    if (!usedRange) {
                        console.log(`[CLEANUP] No used range found in ${assumptionTab.name}`);
                        return;
                    }
                    ...
                    // Start from row 11 as requested
                    const startRow = 11;
                    if (lastRow < startRow) {
                        console.log(`[CLEANUP] ${assumptionTab.name} has less than ${startRow} rows`);
                        return;
                    }
                    
                    // Get column B values from row 11 to last row
                    const columnBRange = worksheet.getRange(`B${startRow}:B${lastRow}`);
                    columnBRange.load("values");
                    await clearContext.sync();
                    
                    const values = columnBRange.values;
                    const cellsToClear = [];
                    ...
                    for (let i = 0; i < values.length; i++) {
                        ...
                        // Check if the cell contains exactly "BR" (case-sensitive)
                        if (cellValue === "BR") {
                            const rowNumber = startRow + i;
                            cellsToClear.push(rowNumber);
                        }
                    }
                    
                    // Clear all BR cells at once
                    if (cellsToClear.length > 0) {
                        for (const rowNumber of cellsToClear) {
                            const cellToClean = worksheet.getRange(`B${rowNumber}`);
                            cellToClean.clear(Excel.ClearApplyTo.contents);
                        }
                        await clearContext.sync();
                        console.log(`[CLEANUP] Cleared ${cellsToClear.length} BR cells in ${assumptionTab.name}: rows ${cellsToClear.join(', ')}`);
                    } else {
                        console.log(`[CLEANUP] No BR cells found in ${assumptionTab.name}`);
                    }
                });
            } catch (error) {
                console.error(`[CLEANUP] Error clearing BR cells in ${assumptionTab.name}:`, error);
            }
        }
        console.log("[CLEANUP] Completed clearing BR cells in all assumption tabs");
...
async function replaceIndirectsJS(worksheet, lastRow) {
    ...
    try {
        ...
        for (const TARGET_COL of TARGET_COLS) {
            
            if (TARGET_COL === "I") {
                // Special sequential processing for column I
                await processColumnISequentially(worksheet, START_ROW, lastRow);
            } else {
                // Original processing for other columns (like U)
                await processColumnOriginalMethod(worksheet, TARGET_COL, START_ROW, lastRow);
            }
            
        }
        
    } catch (error) {
        console.error(`Error in replaceIndirectsJS for sheet ${worksheet.name}:`, error.debugInfo || error);
        throw error;
    }
}

/**
 * Finds the matching closing parenthesis for a given opening parenthesis position
...
/**
 * Sequential processing for column I: single INDIRECT → ROW functions → complex INDIRECT
 */
async function processColumnISequentially(worksheet, startRow, lastRow) {
    const targetRangeAddress = `I${startRow}:I${lastRow}`;
    const targetRange = worksheet.getRange(targetRangeAddress);
    ...
    let formulas = targetRange.formulas.map(f => f[0]); // Convert to 1D array
    
    // Step 1: Replace single cell INDIRECT functions (INDIRECT(A10))
    formulas = await replaceSingleCellIndirects(formulas, worksheet);
    
    // Step 2: Replace ROW functions (ROW(A12) → 12)
    formulas = replaceRowFunctions(formulas, startRow);
    
    // Step 3: Replace complex INDIRECT functions (expressions with multiple references)
    formulas = await replaceComplexIndirects(formulas, worksheet);
    
    // Write back the processed formulas
    const newFormulas = formulas.map(f => [f]); // Convert back to 2D array
    targetRange.formulas = newFormulas;
}

/**
 * Recursively find all INDIRECT references, including nested ones
 */
function findAllIndirectReferences(formula, referencesToLookup) {
    let searchStartIndex = 0;
    while (true) {
        const upperFormula = formula.toUpperCase();
        const indirectStartIndex = upperFormula.indexOf("INDIRECT(", searchStartIndex);
        
        if (indirectStartIndex === -1) break;
        
        const parenStartIndex = indirectStartIndex + "INDIRECT(".length;
        const parenEndIndex = findMatchingCloseParen(formula, indirectStartIndex + "INDIRECT".length);
        
        if (parenEndIndex === -1) break;
        
        const argString = formula.substring(parenStartIndex, parenEndIndex).trim();
        
        console.log(`Step 1: Examining INDIRECT argument: "${argString}"`);
        
        // Check if this is a simple single cell reference
        if (/^[A-Z]+\d+$/i.test(argString) && !referencesToLookup.has(argString)) {
            referencesToLookup.set(argString, { range: null, value: undefined });
            console.log(`Step 1: Found single cell INDIRECT reference: ${argString}`);
        } else {
            console.log(`Step 1: Complex expression, searching recursively: "${argString}"`);
            // Recursively search within this complex expression
            if (argString.toUpperCase().includes('INDIRECT(')) {
                findAllIndirectReferences(argString, referencesToLookup);
            }
        }
...
/**
 * Step 1: Replace single cell INDIRECT functions like INDIRECT(A10)
 */
async function replaceSingleCellIndirects(formulas, worksheet) {
    const referencesToLookup = new Map();
    
    // First pass: identify ALL single cell INDIRECT references (even deeply nested ones)
    // We need to recursively find INDIRECT functions to catch nested ones
    for (let formula of formulas) {
        if (typeof formula === 'string' && formula.toUpperCase().includes('INDIRECT(')) {
            // Use a more thorough approach to find ALL INDIRECT references
            findAllIndirectReferences(formula, referencesToLookup);
        }
    }
    
    // Load all the reference values
    if (referencesToLookup.size > 0) {
        for (const [refString, data] of referencesToLookup.entries()) {
            try {
                ...
                data.range.load(["values", "text"]);
            } catch (error) {
                console.warn(`Error getting range for ${refString}:`, error);
                data.value = '#REF!';
            }
        }
        ...
        for (const [refString, data] of referencesToLookup.entries()) {
            if (data.range) {
                try {
                    ...
                    console.log(`Step 1: ${refString} -> ${data.value}`);
                } catch (error) {
                    console.warn(`Error reading value for ${refString}:`, error);
                    data.value = '#VALUE!';
                }
            }
        }
    ...
    // Second pass: replace ALL single cell INDIRECT functions using regex approach
    const processedFormulas = [];
    for (let formula of formulas) {
        if (typeof formula === 'string' && formula.toUpperCase().includes('INDIRECT(')) {
            let currentFormula = formula;
            let globalLoopCount = 0;
            const MAX_GLOBAL_LOOPS = 10; // Prevent infinite loops
            
            console.log(`Step 1: Processing formula: ${currentFormula}`);
            
            // Keep processing until no more simple INDIRECTs can be replaced
            while (globalLoopCount < MAX_GLOBAL_LOOPS) {
                let foundAndReplaced = false;
                
                // Use regex to find and replace all simple INDIRECT(cellRef) patterns
                for (const [cellRef, data] of referencesToLookup.entries()) {
                    // Create a regex pattern to match INDIRECT(cellRef) exactly
                    const pattern = new RegExp(`INDIRECT\\(${cellRef}\\)`, 'gi');
                    
                    if (pattern.test(currentFormula)) {
                        let directRef = data.value;
                        
                        if (directRef === null || typeof directRef === 'undefined') {
                            directRef = 0;
                        } else if (typeof directRef === 'boolean') {
                            directRef = directRef ? 'TRUE' : 'FALSE';
                        }
                        
                        const beforeReplacement = currentFormula;
                        currentFormula = currentFormula.replace(pattern, String(directRef));
                        
                        console.log(`Step 1: Replacing INDIRECT(${cellRef}) with ${directRef}`);
                        console.log(`Step 1: Before: ${beforeReplacement}`);
                        console.log(`Step 1: After:  ${currentFormula}`);
            ...
            console.log(`Step 1: Result: ${currentFormula}`);
            processedFormulas.push(currentFormula);
        } else {
            processedFormulas.push(formula);
        }
    }
...
/**
 * Step 2: Replace ROW functions with their numeric values
 */
function replaceRowFunctions(formulas, startRow) {
    const processedFormulas = [];
    
    for (let i = 0; i < formulas.length; i++) {
        let formula = formulas[i];
        
        if (typeof formula === 'string' && formula.toUpperCase().includes('ROW(')) {
            console.log(`Step 2: Processing formula: ${formula}`);
            
            // Replace ROW(cellRef) with the row number of cellRef
            formula = formula.replace(/ROW\(([A-Z]+\d+)\)/gi, (match, cellRef) => {
                // Extract row number from cell reference
                const rowMatch = cellRef.match(/\d+$/);
                const replacement = rowMatch ? rowMatch[0] : match;
                console.log(`Step 2: Replacing ${match} with ${replacement}`);
                return replacement;
            });
            
            // Replace ROW() with current row number
            const currentRowNumber = startRow + i;
            formula = formula.replace(/ROW\(\)/gi, String(currentRowNumber));
            
            console.log(`Step 2: Result: ${formula}`);
        }
 ...
 * Step 3: Replace complex INDIRECT functions (expressions with multiple references)
 */
async function replaceComplexIndirects(formulas, worksheet) {
    // This step handles INDIRECT functions that contain concatenated expressions
    // after the single cell INDIRECTs and ROW functions have been resolved
    ...
    for (let formula of formulas) {
        if (typeof formula === 'string' && formula.toUpperCase().includes('INDIRECT(')) {
            ...
            console.log(`Step 3: Processing formula: ${currentFormula}`);
            ...
            while (loopCount < MAX_LOOPS) {
                ...
                const parenStartIndex = indirectStartIndex + "INDIRECT(".length;
                const parenEndIndex = findMatchingCloseParen(currentFormula, indirectStartIndex + "INDIRECT".length);
                
                if (parenEndIndex === -1) break;
                
                const indString = currentFormula.substring(indirectStartIndex, parenEndIndex + 1);
                const argString = currentFormula.substring(parenStartIndex, parenEndIndex).trim();
                
                console.log(`Step 3: Found INDIRECT expression: ${argString}`);
                
                // Evaluate the complex expression
                let resolvedRange = '#REF!';
                try {
                    resolvedRange = await evaluateComplexExpression(argString);
                    console.log(`Step 3: Evaluated "${argString}" to "${resolvedRange}"`);
                } catch (error) {
                    console.warn(`Error evaluating complex expression "${argString}":`, error);
                }
                
                console.log(`Step 3: Replacing ${indString} with ${resolvedRange}`);
                currentFormula = currentFormula.replace(indString, resolvedRange);
                loopCount++;
            }
            
            console.log(`Step 3: Result: ${currentFormula}`);
            processedFormulas.push(currentFormula);
        } else {
            processedFormulas.push(formula);
        }
...
/**
 * Evaluate complex expressions like "U"&28&":CN"&28
 */
async function evaluateComplexExpression(expression) {
    // At this point, all single INDIRECTs and ROW functions should be resolved
    // So we just need to handle string concatenation
    ...
    if (expression.includes('&')) {
        ...
        for (let part of parts) {
            if (part.startsWith('"') && part.endsWith('"')) {
                ...
                // Should be a number by now (from resolved ROW functions)
                result += part;
            }
        }
...
/**
 * Original processing method for other columns (like U)
 */
async function processColumnOriginalMethod(worksheet, targetCol, startRow, lastRow) {
    const targetRangeAddress = `${targetCol}${startRow}:${targetCol}${lastRow}`;
    const targetRange = worksheet.getRange(targetRangeAddress);
    
    targetRange.load("formulas");
    await worksheet.context.sync();

    const originalFormulas = targetRange.formulas;
    const referencesToLookup = new Map();
    const formulaData = [];
    
    const hasIndirect = originalFormulas.some(formula => 
        typeof formula[0] === 'string' && formula[0].toUpperCase().includes('INDIRECT(')
    );
    ...
    // First Pass: Identify INDIRECT arguments
    for (let i = 0; i < originalFormulas.length; i++) {
        let formula = originalFormulas[i][0];
        formulaData.push({ originalFormula: formula, index: i });
        ...
        if (typeof formula === 'string') {
            ...
            while (true) {
                ...
                if (parenEndIndex === -1) {
                    console.warn(`Row ${startRow + i}: Malformed INDIRECT found in formula: ${formula}`);
                    break;
                }
    ...
    if (referencesToLookup.size > 0) {
        for (const [refString, data] of referencesToLookup.entries()) {
            try {
                ...
                console.warn(`Replace_Indirects: Error getting range for reference "${refString}".`, rangeError.debugInfo || rangeError.message);
                data.value = '#REF!';
            }
        }
        await worksheet.context.sync();
    ...
    for (const item of formulaData) {
        ...
        if (typeof currentFormula === 'string') {
            ...
            while (loopCount < MAX_LOOPS) {
                ...
                let directRef = '#REF!';
                 if (referencesToLookup.has(argString)) {
                     ...
                     console.warn(`Row ${startRow + item.index}: INDIRECT argument "${argString}" not found in lookup map during replacement.`);
                 }
            ...
            if (loopCount === MAX_LOOPS) {
                console.warn(`Row ${startRow + item.index}: Max replacement loops reached for formula. Result might be incomplete: ${currentFormula}`);
            }
        }
