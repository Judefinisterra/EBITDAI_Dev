...
// Direct API call function that handles both OpenAI and Claude models
async function* callOpenAIForModelPlanner(messages, options = {}) {
  const { model = "claude-opus-4-1-20250805", temperature = 0.7, stream = false } = options;

  // Check if it's a Claude model
  const isClaudeModel = model.includes('claude');
  
  if (isClaudeModel) {
    // Import and use callOpenAI from AIcalls which handles Claude
    const { callOpenAI } = await import('./AIcalls.js');
    
    // Convert messages format for Claude if needed
    const claudeOptions = {
      model,
      temperature,
      stream,
      useClaudeAPI: true,
      caller: 'AIModelPlanner'
    };
    
    console.log(`AIModelPlanner - Using Claude API with model: ${model}`);
    
    // callOpenAI already returns an async generator
    yield* callOpenAI(messages, claudeOptions);
    return;
  }
...
async function handleInitialAIModelPlannerConversation(userInput) {
    ...
    // Get selected model from dropdown, default to claude-opus-4-1-20250805
    // Check both client mode and dev mode dropdowns
    const appBody = document.getElementById('app-body');
    const isDevMode = appBody && appBody.style.display !== 'none';
    const modelDropdown = isDevMode 
        ? document.getElementById('dev-model-dropdown') 
        : document.getElementById('model-dropdown');
    const model = modelDropdown ? modelDropdown.value : "claude-opus-4-1-20250805";
    console.log("AIModelPlanner: Using model:", model);
